...
def k_SRBF(lat,lon,k_init,min_p,min_d,max_d,R,workers):
    ...
    car_x,car_y,car_z = other_functions.sph2car(lat_all,lon_all,R)
    ...
    lat_rbf,lon_rbf = other_functions.car2sph(carx_rbf,cary_rbf,R,car_x,car_y,car_z)
    idx = kmeans.labels_
##############################################################################    
    # m = len(lat_rbf)
...
##############################################################################  
    ...
    while next_s == 1:
        ...
        if indis1+1 == m and m>1:
            ...
            lat_rbf1,lon_rbf1 = other_functions.car2sph(carx_rbf1,cary_rbf1,R,car_x,car_y,car_z)
...
def depth_sel_with_GCV_ind(lat_sph,lon_sph,rad_dist,lat_srbf,lon_srbf,residual_data,R_bjerhammer,function,workers,depth_range,regu,norm_fac,total_data):
    ...
    for j in depth_range:
        ...
        coeff_mat[0] = coeff_matrix.other_kernel_coeff_mat(rad_dist[0],lat_sph[0],lon_sph[0],0,rad_dist_rbf,function[0],lat_srbf,lon_srbf,workers)
        #print('\nParameter Estimation started. Please wait...')
        # parameter = other_functions.LS(coeff_mat,residual_data)
        parameter = np.linalg.solve(np.dot(coeff_mat[0].T,coeff_mat[0]),np.dot(coeff_mat[0].T,residual_data[0]))
            
        # all_coeff = np.concatenate((coeff_mat[:]))
        # coeff_mat = [None]*data_number
        # all_res = np.concatenate((residual_data[:]))
        
        # Beta = np.linalg.solve(np.dot(all_coeff.T,all_coeff),all_coeff.T)
        # trace = np.dot(all_coeff,Beta)
        # trace = np.dot(all_coeff,(np.dot(all_coeff.T,all_coeff)**-1)*all_coeff.T) 
        # gcv[k] = (total_data*np.sum(((np.dot(all_coeff,parameter)-all_res)*1e-5)**2)) / ((total_data-np.trace(trace))**2)
        # res[k] = np.dot(all_coeff,parameter)
        trace = np.dot(coeff_mat[0],(np.dot(coeff_mat[0].T,coeff_mat[0])**-1)*coeff_mat[0].T) 
        gcv[k] = (total_data*np.sum(((np.dot(coeff_mat[0],parameter)-residual_data[0])*1e-5)**2)) / ((total_data-np.trace(trace))**2)
        res[k] = np.dot(coeff_mat[0],parameter)
    ...
    return(depth_last,gcv,res_last)
