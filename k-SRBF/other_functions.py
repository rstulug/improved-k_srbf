...
def car2sph(x,y,R,x1,y1,z1):
    ...
    if any(z1<0):
        for i in range(len(fi)):
            nearest = np.argmin(np.sqrt((x[i]-x1)**2 + (y[i]-y1)**2))
            if z1[nearest]<0:
                fi[i] *= -1
    return(fi,lamda)
...
def ell2car(fi,lamda,h):
    ...
    x = (N+h)*np.cos(lamda)*np.cos(fi)
    y = (N+h)*np.cos(fi)*np.sin(lamda)
    z = ((b**2/a**2)*N+h)*np.sin(fi)
...
# def LS2(coeff_mat,grav):
#     data_number = len(coeff_mat)
#     normal_matrix = [None]*data_number
#     obs_vector = [None]*data_number
#     r = np.zeros(data_number)
#     sigma = [np.array([1])]*(data_number)
#     sigma_dif = np.zeros(data_number)
#     u2 = [None]*data_number
#     for i in range(data_number):
#         normal_matrix[i] = (coeff_mat[i].T @ coeff_mat[i])
#         obs_vector[i] = np.dot(coeff_mat[i].T,grav[i])
...
#         for i in range(data_number):
#             sigma_dif[i] = sigma[i][-1] - sigma[i][-2]               
#         if np.all(np.abs(sigma_dif)<0.001):
#             return(parameter,sigma)
...
def LS2(coeff_mat,grav):
    ...
    while True:
        ...
        for i in range(data_number):
            Beta = np.linalg.solve(Qd,((1/sigma[i][-1]**2)*np.dot(coeff_mat[i].T,coeff_mat[i])))
            r[i] = len(coeff_mat[i])-np.trace(Beta)
