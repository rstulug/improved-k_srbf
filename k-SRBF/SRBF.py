...
# -*- coding: utf-8 -*-
"""
Created on Fri Dec 14 15:26:54 2019
...
nw_first = time.time()
##############################################################################
print("\nk-SRBF: The Regional Gravity Field Modeling Software using Spherical Radial Basis functions\n")
...
for i in range(data_number):
    ...
    print('\nData File format: "Point Id" "Latitude" "Longitude" "Height (meter)" "Data(Anomaly,Disturbance (mgal), etc.)"')
    ...
    print('\n1: Gravity Anomaly (mgal) \n2: Gravity Disturbance (mgal)')
...
##############################################################################
control_data = 2
print('\nDo you wanna use external control points, to validate results')
print('\nNote:If you want to determine optimal bandwidth and minimum resolution, you have to give external control points')
...
Lmin =  int(input('\nEnter degree of Global Geopotential Model for long-wavelength computation\n: '))
...
##############################################################################
print("\nIn this software Residual Terrain Effects is calculated using Gravsoft TC module as subprocess")
...
min_srbf = float(input('\nPlease enter the minimum spherical distance between the centroids: '))
max_srbf = float(input('\nPlease enter the maximum spherical distance between centroid and its samples to split the cluster: '))
...
if depth_choose == 2 and control_data !=1:
    raise Exception("Without external control data, you cannot select a depth range")

if depth_choose == 2:
    depth_i,depth_l,depth_s = input('\nEnter depth range (km) (Initial Last Step Size (e.g. 5 20 1)): ').split()
    try:
...
##############################################################################
if qgeoid == 1:    
    print('\nHere we follow turning point algorithm developed (see doi: 10.1016/j.jog.2019.01.001)')
    print('\nUser will choose turning point from graph if reduced SRBF range option selected')
...
nw = time.time()
lat_srbf,lon_srbf = k_SRBF.k_SRBF(lat_sph,lon_sph,k_init,k_min,min_srbf,max_srbf,workers)
gc.collect()  
...
if regu == 0:
    print('Normal equation matrix is nonsingular, least-square solution will be performed without regularization')
else:
    print('Normal equation matrix is singular, least-square solution will be performed with MCVCE')
coeff_mat = [None]*data_number
gc.collect()
##############################################################################
if depth_choose == 2:
...
if Lmin_choose == 2:  
    ...
    print('\nVariance-Component Estimation started...')   
    nw = time.time()
    if regu == 0:
        parameter = other_functions.LS(coeff_mat,residual_data)
    else: 
    ...
    coeff_mat = [None]*data_number
    gc.collect()
    print('\nVariance-Component Estimation completed.Process time: {:.2f} minute'.format((time.time()-nw)/60))
##############################################################################
...
if qgeoid == 1:
    ...
    qgeoid_lat_sph,qgeoid_lon_sph,R_qgeoid = other_functions.car2sph(cart_ell_qgeoid[:,0],cart_ell_qgeoid[:,1],cart_ell_qgeoid[:,2])
...
with open('output/Results.txt','w') as sonuclar:
    ...
    sonuclar.write('Maximum degree expansion of GGM: {}\n'.format(Lmin))
    ...
    if depth_choose == 2:   
        sonuclar.write('\nTested Depth\n')
        sonuclar.write('  {:4s}      {:10s}\n'.format('Depth', 'GCV'))
        for i in range(len(depth_range)):
            sonuclar.write('{:10.3f}    {:10.5e}\n'.format(depth_range[i],gcv[i]))
            
    if Lmin_choose == 2:
        sonuclar.write('\nTested Lmin\n')
        sonuclar.write('  {:4s}      {:10s}\n'.format('Lmin', 'RMS of Control Points'))
        for i in range(len(Lmin2)):
            sonuclar.write('{:10.3f}    {:10.3f}\n'.format(Lmin2[i],rms_control3[i]))
...
if control_data==1:
    with open('output/Control Points.txt','w') as sonuclar:   
        sonuclar.write('{:4s}    {:10s}  {:10s}     {:10s}\n'.format('Point ID','Latitude','Longitude','Estimated Value (mGal)'))
        for i in range(len(lat_control)):
            sonuclar.write('{:4d}      {:10f}   {:10f}    {:10f}\n'.format(i,lat_control[i],lon_control[i],estimated_control[i]))
else:
    pass
                
if qgeoid == 1:       
    with open('output/Height_anomaly.txt','w') as sonuclar:  
        sonuclar.write('  {:4s}    {:10s}    {:10s}   {:12s}   {:10s}\n'.format('no','Latitude','Longitude','Height','Quasi-Geoid Ondulation (m)'))
        for i in range(len(qgeoid_lat)):
            sonuclar.write('{:4d}    {:10f}    {:10f}    {:10f}    {:10f}\n'.format(i,qgeoid_lat[i],qgeoid_lon[i],qgeoid_H[i],estimated_qgeoid[i]))
else:
    pass 


with open('output/SRBF','w') as sonuclar:
    sonuclar.write('  {:4s}    {:10s}    {:10s}    {:10s}    {:10s}\n'.format('no','Latitude','Longitude','Depth(m)','Magnitude '))
    for i in range(len(lat_srbf)):
        sonuclar.write('{:4d}    {:10f}    {:10f}    {:10.3f}    {:10f}\n'.format(i,lat_srbf[i],lon_srbf[i],depth_last,parameter[i]))    
gc.collect()
